...
class Sheet(object):
    ...
    def __init__(self, base_name, wb_sheet, srv_writer, clt_writer):
        ...
        if None is match_list or 1 != len(match_list):
    ...
    def raise_error(self, what, val):
        excel_info = format(
            "DOC:%s,SHEET:%s,ROW:%d,COLUMN:%d" % (self.base_name, self.wb_sheet.title, self.error_row, self.error_col))
    ...
    def to_value(self, val_type, val):
        try:
            ...
            t, e = sys.exc_info()[:2]
            raise Exception("Invalid convert: " + e)
    ...
    def decode_sheet(self):
        ...
        print("covert successfully... sheet name -> %s " % wb_sheet.title)
        return True
...
class ExcelDoc:
    ...
    def decode(self, srv_path, clt_path, srv_writer, clt_writer):
        ...
        for wb_sheet in wb.worksheets:
            SheetDecoder = self.need_decode(wb_sheet)
            if None is SheetDecoder:
                print("covert skip excel........... sheet name -> ", wb_sheet.title)
                continue

            sheet = SheetDecoder(base_name, wb_sheet, srv_writer, clt_writer)
