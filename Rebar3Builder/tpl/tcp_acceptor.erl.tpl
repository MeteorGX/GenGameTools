...
%% @doc 切换 Acceptor 状态
handle_cast({change_closed, Closed}, State = #tcp_acceptor_state{}) ->
  io:format("Change Acceptor Closed = ~w~n", [Closed]),
  {?noreply, State#tcp_acceptor_state{
    closed = ?IF(Closed =:= ?true, ?true, ?false)
  }};
