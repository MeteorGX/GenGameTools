...
-spec start_link(Opts, Listener, Socket) -> gen_server:start_ret()
  when Listener :: port(),
  Socket :: port(),
  Opts :: list().
start_link(Opts, Listener, Socket) -> gen_server:start_link(?MODULE, [
  Opts, Listener, Socket
], []).
...
%% @doc 初始化函数
init([Opts, Listener, Socket]) ->
  sys_utils:set_trap_exit(),

  % 检索内部回调
  OnEstablished = lists:keyfind(?on_established, 1, Opts),
  OnMessage = lists:keyfind(?on_message, 1, Opts),
  OnClosed = lists:keyfind(?on_closed, 1, Opts),
